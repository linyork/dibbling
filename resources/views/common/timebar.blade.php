        ...
        <div class="container">
            <div class="form-group row">
                ...
                <div class="input-group offset-lg-2 col-lg-2 offset-md-8 col-md-4">
                    <select class='custom-select order-list' name="order">
                        <option value="0" {{ $order == '0' ? 'selected' : '' }}>{{ __('web.order.All') }}</option>
                        <option value="1" {{ $order == '1' ? 'selected' : '' }}>{{ __('web.order.Dibbling') }}</option>
                        <option value="2" {{ $order == '2' ? 'selected' : '' }}>{{ __('web.order.ReDibbling') }}</option>
                        <option value="3" {{ $order == '3' ? 'selected' : '' }}>{{ __('web.order.Cut') }}</option>
                        <option value="5" {{ $order == '5' ? 'selected' : '' }}>{{ __('web.order.Liked') }}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-group row justify-content-end">
                <div class="col-auto">
                    ...
                    <a href="{{ route('timeline') }}" class="btn btn-secondary">{{ __('web.record.Reset') }}</a>
                    <button type="summit" class="btn btn-info">{{ __('web.record.Search') }}</button>
                </div>
            </div>
