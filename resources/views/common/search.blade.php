            ...
            <div class="form-group row">
                <div class="input-group col-3">
                    ...
                    <select class="custom-select" id="user_id">
                        ...
                        @foreach(\App\Model\UserModel::select(\DB::raw('count(record.user_id) as count'), \DB::raw('users.*'))->leftJoin('record', 'users.id', '=', 'record.user_id')->where('record.record_type', \App\Model\RecordTable::DIBBLING)->orWhere('record.record_type', NULL)->groupBy('users.id')->orderBy('count', 'DESC')->get() as $user)
